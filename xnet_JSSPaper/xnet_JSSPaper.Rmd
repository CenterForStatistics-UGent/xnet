...
author:
  ...
  - name: Bernard De Baets
    ...
    url: https://www.ugent.be/bw/damm/en
title:
  formatted: "Predicting Networks \\corr{with} Two-Step Kernel Ridge Regression: The R Package \\pkg{xnet}"
...
abstract: >
  This paper presents the R package \pkg{xnet} for supervised network prediction, using two-step kernel ridge regression. It uses the cross-validation shortcuts \corr{developed} by @Stock2018cvshortcuts to allow for computationally efficient validation of the models based on a variety of leave-one-out cross-validation methods. The package provides functions for easy tuning, fitting, and evaluation of two-step kernel ridge regression \corr{for} supervised network prediction. We illustrate the use of the \pkg{xnet} package with datasets from different areas of research.
keywords:
