...
Even though computational speed is important, much attention was given to the intuitivity of the API design. Using 


## Data overview

The dataset `drugtarget`, first presented by @Yamanishi2008, serves as an example of a bipartite network. It is a small dataset containing all binary interactions between 26 nuclear receptor protein targets and 54 drug-like compounds. As the SIMCOMP algorithm used by @Yamanishi2008 doesn't necessarily return a symmetric matrix, the similarities between the chemical compounds were recalculated based on the Tanimoto coefficient, using the \pkg{fmcsR} package. **INCLUDE REFERENCES**
...
The example dataset `proteinInteraction` originates from a publication by @Yamanishi2004. It contains an interaction network between a set of 769 proteins, and consists of two matrices:
...
One can fit a two-step kernel ridge regression using the function `tskrr()`. In addition to the interaction matrix and the kernel matrices, this function accepts the tuning parameters `lambda`, as explained earlier.  The user can choose to set one lambda for tuning $K$ and $G$ using the same lambda value, or they can specify a different lambda for $K$ and $G$. If none are provided, the function uses default values of $10^{-4}$ for $\lambda_k$ and $\lambda_g$[^defaulthp].
